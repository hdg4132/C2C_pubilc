...
import { Route, Routes } from 'react-router-dom'; // 페이지 이동을 도와주는 기능들을 가져옴
...
const App = () => { 
  ...
  useEffect(() => { // 컴포넌트가 화면에 처음 나타날 때 실행되는 부분
    async function resData() { // 비동기로 서버에서 데이터를 가져오는 함수
      const movies = await axios.get(`http://localhost:5000/movie`, {}); // 서버에서 영화 데이터를 가져옴
  ...
  return ( // 화면에 보여줄 내용을 작성하는 부분입니다.
    <div className="container"> 
      <div className="banner"> 
        <img src="/main_banner.png" alt="Cinema tickets banner" /> 
      </div>
      <Routes>  
        <Route path="/" element={
          <QuickBooking
